...
import memgcn
from rev_layer import SharedDropout
import copy
...
class R_GATConv(nn.Module):
    def __init__(
        ...
        self.attn_l = torch.normal(mean=0, std=1.0,size=(1, num_heads, out_feats)).cuda()
        self.attn_l = F.normalize(self.attn_l,dim=-1,p=2.0)
        print('attn_l is randomly chosen!')
        #print('attn_l is learning!')
        if use_attn_dst:
            #self.attn_r = nn.Parameter(torch.FloatTensor(size=(1, num_heads, out_feats)))
            self.attn_r = torch.normal(mean=0, std=1.0,size=(1, num_heads, out_feats)).cuda()
            self.attn_r = F.normalize(self.attn_r,dim=-1,p=2.0)
            print('attn_r is randomly chosen!')
        else:
...
class R_RevGATBlock(nn.Module):
    def __init__(
        ...
        super(R_RevGATBlock, self).__init__()
