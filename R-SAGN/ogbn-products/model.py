        ...
        if self.base_model is not None:
            ...
            if hasattr(self.base_model, "hop_attn_l"):
                if self.base_model._weight_style == "attention":
                    ...
                    #================
                    self.base_model.hop_attn_l = torch.rand(size=(1, self.base_model._num_heads,
                                                                  self.base_model._hidden)).cuda()
                    self.base_model.hop_attn_r = torch.rand(size=(1, self.base_model._num_heads, 
                                                                  self.base_model._hidden)).cuda()
                    
                    #self.base_model.hop_attn_l = torch.normal(mean=0, std=1.0,size=(1, num_heads,hidden)).cuda()
                                                                                     
                    #self.base_model.hop_attn_r = torch.normal(mean=0, std=1.0,size=(1, num_heads,hidden)).cuda()
                                                                                     
                    
                    self.base_model.hop_attn_l = F.normalize(self.base_model.hop_attn_l,dim=-1,p=2.0)
                    self.base_model.hop_attn_r = F.normalize(self.base_model.hop_attn_r,dim=-1,p=2.0)
                    print('Random Attention Again!')
                    #=================
...
class R_SAGN(nn.Module):
    def __init__(self, in_feats, hidden, out_feats, num_hops, n_layers, num_heads, weight_style="attention", alpha=0.5, focal="first",
        ...
        if weight_style == "attention":
            ...
            #================
            self.hop_attn_l = torch.rand(size=(1, num_heads, hidden)).cuda()
            self.hop_attn_r = torch.rand(size=(1, num_heads, hidden)).cuda()
            
            #self.hop_attn_l = torch.normal(mean=0, std=1.0,size=(1, num_heads, hidden)).cuda()
            #self.hop_attn_r = torch.normal(mean=0, std=1.0,size=(1, num_heads, hidden)).cuda()
            
            
            self.hop_attn_l = F.normalize(self.hop_attn_l,dim=-1,p=2.0)
            self.hop_attn_r = F.normalize(self.hop_attn_r,dim=-1,p=2.0)
            print('Random Attention Please!')
            #=================
